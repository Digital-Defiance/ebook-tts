...
from ..workspace.manifests import read_planned_chunk_text, verify_plan_artifacts
...
def run_manifest_fingerprint(manifest: dict[str, Any]) -> str:
  """Fingerprint the exact canonical run state covered by one QA report."""
  return sha256_text(canonical_json(manifest))


def quality_report_id(
    run_id: str,
    run_manifest_sha256: str,
    configuration: dict[str, Any],
) -> str:
  """Return the content-derived identity for one QA policy and run state."""
  return sha256_text(
      canonical_json(
          {
              "run_id": run_id,
...
def _validated_chunks(
  ...
  if chunk_directory.is_symlink():
    raise QualityError(f"Track {track_number} chunk directory is a symbolic link.")
  for position, (raw_planned, raw_generated, reference) in enumerate(
    ...
    sidecar_path = chunk_path.with_suffix(chunk_path.suffix + ".json")
    if chunk_path.is_symlink() or sidecar_path.is_symlink():
      raise QualityError(
          f"Track {track_number}, chunk {position} path uses a symbolic link."
      )
    ...
    if (
        ...
        or stored_media.get("bit_rate") != chunk_media.bit_rate
...
def evaluate_run(
  ...
  with WorkspaceLock(plan.workspace):
    ...
    current_run = load_run(plan, run.run_id)
    run_manifest = current_run.manifest
    run = current_run
    run_root = run.run_path.parent
    run_manifest_sha256 = run_manifest_fingerprint(run_manifest)
    qa_id = quality_report_id(
        run.run_id,
        run_manifest_sha256,
        qa_configuration,
    )
    qa_parent = run_root / "qa"
    qa_root = qa_parent / qa_id
    report_path = qa_root / "report.json"
    html_path = qa_root / "report.html"
    if qa_parent.is_symlink() or qa_root.is_symlink():
      raise WorkspaceError("QA report path must not traverse symbolic links.")
    if report_path.is_symlink() or html_path.is_symlink():
      raise WorkspaceError("QA report files must not be symbolic links.")
    if (qa_parent.exists() and not qa_parent.is_dir()) or (
        qa_root.exists() and not qa_root.is_dir()
    ):
      raise WorkspaceError("QA report path is not a directory.")
    if (
    ...
    if plan_adopted:
      plan_adoption = plan.manifest.get("adoption")
      run_adoption = run_manifest.get("adoption")
      if not isinstance(plan_adoption, dict) or run_adoption != plan_adoption:
        raise WorkspaceError("Adopted plan and run provenance disagree.")
      adoption_hash = plan_adoption.get("adoption_sha256")
      adoption_core = dict(plan_adoption)
      adoption_core.pop("adoption_sha256", None)
      if (
          not isinstance(adoption_hash, str)
          or not _SHA256.fullmatch(adoption_hash)
          or sha256_text(canonical_json(adoption_core)) != adoption_hash
      ):
        raise WorkspaceError("Adoption provenance fingerprint is invalid.")
      adoption_path = plan.workspace / "adoption.json"
      if adoption_path.is_symlink() or not adoption_path.is_file():
        raise WorkspaceError("Adoption record is missing or unsafe.")
      adoption_record = load_json(adoption_path)
      if (
          adoption_record.get("manifest_version") != MANIFEST_VERSION
          or adoption_record.get("kind") != "ebook-tts-adoption"
          or adoption_record.get("plan_sha256") != plan.plan_id
          or adoption_record.get("run_id") != run.run_id
          or any(adoption_record.get(key) != value for key, value in plan_adoption.items())
      ):
        raise WorkspaceError("Adoption record does not match the selected plan and run.")
    failures: list[str] = []
    warnings: list[str] = []
    ...
    for track_number in sorted(plan_sections):
      ...
      try:
        if plan_adopted:
          if final_path.parent.is_symlink() or final_path.is_symlink():
            raise QualityError("final audio path traverses a symbolic link")
          ...
          if (
              ...
              or final_record.get("bit_rate") != media.bit_rate
          ...
          media, assembly_sha256 = verify_native_final_checkpoint(
              plan=plan,
              run=run,
              section=section,
              state=state,
